import { Home, Search, Bell, Mail, User, MoreHorizontal, LogOut } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside className="flex flex-col justify-between h-screen w-96 border-r border-gray-200 p-4">
      ...
      <nav className="flex flex-col justify-center gap-4 self-center">
        <SidebarItem icon={<Home />} label="Inicio" active onClick={() => navigate("/")} />
        <SidebarItem icon={<Search />} label="Explorar" onClick={() => navigate("/explore")} />
        <SidebarItem icon={<Bell />} label="Notificaciones" />
        <SidebarItem icon={<Mail />} label="Mensajes" />
      ...
      <button className="bg-black text-white rounded-full py-2 px-4 mt-8 transition md:w-full">
